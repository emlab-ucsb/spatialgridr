...
test_that("returns Samoa example of raw data  - sf", {
  expect_s3_class(suppressWarnings(get_data_in_grid(boundary = get_boundary(name = "Samoa", type = "eez", country_type = "country"),
                                                         dat = readRDS(system.file("extdata", "ridges.rds", package = "spatialgridr")))),
                  class = "sf")
...
test_that("returns kiribati example (antimeridian example) of raw data - sf", {
  expect_s3_class(suppressWarnings(get_data_in_grid(boundary = get_boundary(name = "Kiribati", type = "eez", country_type = "sovereign"),
                                                         dat = readRDS(system.file("extdata", "ridges.rds", package = "spatialgridr")),
                                                         antimeridian = TRUE)),
                  class = "sf")
